...
switch(typeof e) {
	...
	case"number": case"string": return e;
...
return this.gp.leaderboard.open( {
	id: this.gp.player.id,orderBy: e.split(",").map((e=>e.trim())).filter((e=>e)), order: t, limit: s, withMe: r, includeFields: n.split(",").map((e=>e.trim())).filter((e=>e)), displayFields: i.split(",").map((e=>e.trim())).filter((e=>e))
}
	).catch(console.warn)
}
LeaderboardFetch(e,t,s,r,n,i) {
return this.gp.leaderboard.fetch( {
	id: this.gp.player.id,orderBy: t.split(",").map((e=>e.trim())).filter((e=>e)), order: s, limit: r, withMe: n, includeFields: i.split(",").map((e=>e.trim())).filter((e=>e))
}
).then((t=> {
...
return this.gp.leaderboard.fetchPlayerRating( {
	id: this.gp.player.id,orderBy: t.split(",").map((e=>e.trim())).filter((e=>e)), order: s
}
).then((t=> {
...
return this.gp.leaderboard.openScoped( {
	...h,variant: t,order: s,limit: r,includeFields: n.split(",").map((e=>e.trim())).filter((e=>e)), displayFields: i.split(",").map((e=>e.trim())).filter((e=>e)), withMe: a
}
	).catch(console.warn)
}
LeaderboardScopedFetch(e,t,s,r,n,i) {
const a=parseInt(e,10)||0, l=a>0? {
	id: a
}
: {
	tag: e
...
return this.gp.leaderboard.fetchScoped( {
	...l,variant: t,order: s,limit: r,includeFields: n.split(",").map((e=>e.trim())).filter((e=>e)), withMe: i
}
).then((s=> {
...
...g,variant: t,override: Boolean(s), record: {
	[r]: n, [i]: a, [l]: h
}
}
).then((()=> {
...
return this.gp.leaderboard.fetchPlayerRatingScoped( {
	...n,variant: t,includeFields: s.split(",").map((e=>e.trim())).filter((e=>e))
}
).then((s=> {
...
this.gp.documents.fetch( {
	type: "PLAYER_PRIVACY_POLICY",format: "TXT"
}
	)
}
FilesUpload(e) {
...
this.gp.files.uploadUrl( {
	url: e,filename: t,tags: s.split(",").map((e=>e.trim())).filter((e=>e))
}
).then((e=> {
...
this.gp.files.uploadContent( {
	content: e,filename: t,tags: s.split(",").map((e=>e.trim())).filter((e=>e))
}
).then((e=> {
...
this.gp.channels.kick( {
	channelId: e,playerId: t
}
	)
}
Channels_Mute_UnmuteAt(e,t,s) {
this.gp.channels.mute( {
	channelId: e,playerId: t,unmuteAt: s
}
	)
}
Channels_Mute_Seconds(e,t,s) {
this.gp.channels.mute( {
	channelId: e,playerId: t,seconds: Number(s)
}
	)
}
Channels_UnMute(e,t) {
...
this.gp.channels.sendMessage( {
	channelId: e,text: t,tags: s.split(",").map((e=>e.trim())).filter((e=>e))
}
	)
}
Channels_SendPersonalMessage(e,t,s) {
this.gp.channels.sendPersonalMessage( {
	playerId: e,text: t,tags: s.split(",").map((e=>e.trim())).filter((e=>e))
}
	)
}
...
this.gp.channels.fetchMessages( {
	channelId: e,tags: t.split(",").map((e=>e.trim())).filter((e=>e)), limit: s, offset: r
}
).then((e=> {
...
this.gp.channels.fetchPersonalMessages( {
	playerId: e,tags: t.split(",").map((e=>e.trim())).filter((e=>e)), limit: s, offset: r
}
).then((e=> {
...
this.gp.channels.fetchMoreMessages( {
	channelId: e,tags: t.split(",").map((e=>e.trim())).filter((e=>e)), limit: s
}
).then((e=> {
...
this.gp.channels.fetchMorePersonalMessages( {
	playerId: e,tags: t.split(",").map((e=>e.trim())).filter((e=>e)), limit: s
}
).then((e=> {
